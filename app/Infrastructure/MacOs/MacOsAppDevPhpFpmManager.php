...
/**
 * @mago-expect lint:cyclomatic-complexity Version discovery, publication, and exact recovery use explicit gates.
 * @mago-expect lint:excessive-parameter-list Explicit transport, rendering, layout, launchd, and lock boundaries are required.
 * @mago-expect lint:kan-defect Version discovery, publication, and exact recovery use explicit gates.
 * @mago-expect lint:too-many-methods Narrow methods keep installation, publication, launchd, and recovery steps distinct.
 */
final readonly class MacOsAppDevPhpFpmManager implements AppDevPhpFpmManager
{
    public function __construct(
        private AppDevSiteRepository $sites,
        private MacOsAppDevPhpFpmConfigRenderer $renderer,
    ...
    public function converge(Node $node): void
    {
        $this->lock->synchronized($node->id, function () use ($node): void {
            ...
            $storedVersions = $sites
                ->map(static fn (\App\Infrastructure\AppDev\AppDevSite $site): string => $site->phpVersion)
                ->unique()
                ->sort()
                ->values();

            foreach ($storedVersions as $storedVersion) {
                $this->assertSupportedVersion($storedVersion);
            }

            $versions = $storedVersions
                ->merge($this->managedVersions($node, $home))
                ->unique()
                ->sort()
                ->values();
            ...
            foreach ($versions as $version) {
                ...
                $this->convergeVersion($node, $version, $sites->where('phpVersion', $version)->values());
            }
        });
    }

    /** @return list<string> */
    private function managedVersions(Node $node, string $home): array
    {
        $result = $this->execute($node, new RemoteCommand(
            arguments: ['/bin/bash', '-seu', '--', "{$home}/.orbit/php"],
            input: <<<'BASH'
                root=$1
                if [ ! -e "$root" ]; then exit 0; fi
                test -d "$root"
                test ! -L "$root"
                for configuration in "$root"/*/php-fpm.conf; do
                    if [ ! -e "$configuration" ] && [ ! -L "$configuration" ]; then continue; fi
                    version=$(basename "$(dirname "$configuration")")
                    case "$version" in
                        [0-9].[0-9]) printf '%s\n' "$version" ;;
                        *) exit 1 ;;
                    esac
        ...
        if (! $result->succeeded()) {
            throw $this->failure('php-discovery', 'app-dev.php_discovery_failed', $result);
        }

        $versions = array_values(array_filter(explode("\n", trim($result->stdout))));

        foreach ($versions as $version) {
            if (! in_array(needle: $version, haystack: SupportedPhpVersion::all(), strict: true)) {
                throw new RuntimeConvergenceException(
                    step: 'php-version',
                    errorCode: 'app-dev.php_version_unsupported',
                    message: "Managed PHP version [{$version}] is not supported.",
                );
            }
        }
    ...
    private function assertSupportedVersion(string $version): void
    {
        if (in_array(needle: $version, haystack: SupportedPhpVersion::all(), strict: true)) {
            return;
        }
        ...
        throw new RuntimeConvergenceException(
            step: 'php-version',
            errorCode: 'app-dev.php_version_unsupported',
            message: "PHP version [{$version}] is not supported.",
        );
    }

    /**
     * @param \Illuminate\Support\Collection<int, \App\Infrastructure\AppDev\AppDevSite> $sites
     * @mago-expect lint:halstead The method preserves installation, publication, and launchd recovery order.
     */
    private function convergeVersion(Node $node, string $version, \Illuminate\Support\Collection $sites): void
    {
        $home = $this->paths->home($node, RoleName::AppDev);
        $prefix = $this->brewPrefix($node);
        $formula = $version === '8.5' ? 'php' : "php@{$version}";
        $formulaRoot = "{$prefix}/opt/{$formula}";
        $brew = "{$prefix}/bin/brew";
        $token = bin2hex(random_bytes(12));
        $lockPath = $this->layout->phpLock($home, $version);
        $this->acquireTargetLock($node, $home, $version, $lockPath, $token);
        $lease = ['lock_path' => $lockPath, 'token' => $token];
        $primaryFailure = null;

        try {
            $installed = $this->executeWithTargetLease(
                $node,
            ...
            if (! $installed->succeeded()) {
                $installation = $this->executeWithTargetLease(
                    $node,
                ...
                if (! $installation->succeeded()) {
                    throw $this->failure('php-install', 'app-dev.php_install_failed', $installation);
                }
            }

            $discovered = $this->executeWithTargetLease(
                $node,
                ["{$formulaRoot}/bin/php-config", '--version'],
                $lockPath,
                $token,
            );
            ...
            if (
                ...
                || preg_match('/\A'.preg_quote($version, delimiter: '/').'(?:[.]|\z)/D', trim($discovered->stdout))
                    !== 1
            ) {
                throw $this->failure('php-version', 'app-dev.php_version_mismatch', $discovered);
            }

            $configuration = $this->renderer->render($version, $sites, $home, $node->ssh_user, $prefix);
            $label = "com.orbit.php-fpm.{$version}";
            ...
            try {
                $publication = $this->publish(
                    ...
                    plistContents: $this->plist($node, $version, $formulaRoot, $prefix, $home),
                    phpFpm: "{$formulaRoot}/sbin/php-fpm",
                    liveConfiguration: $this->layout->phpCurrent($home, $version),
                    plist: $plist,
                    lockPath: $lockPath,
                    token: $token,
                );
            } catch (Throwable) {
                $this->restorePublishedState($node, $version, $home, $label, $plist, $token, $state, $lease);

                throw new RuntimeConvergenceException(
                    step: 'php-fpm-config',
                    errorCode: 'app-dev.php_fpm_config_failed',
                    message: 'The macOS PHP-FPM operation failed.',
                );
            }

            if ($publication->exitCode === 76) {
                throw $this->failure('php-fpm-rollback', 'app-dev.php_fpm_rollback_failed', $publication);
            }

            if (! $publication->succeeded()) {
                $publicationFailure = $this->failure(
                    'php-fpm-config',
                    'app-dev.php_fpm_config_failed',
                    $publication,
                );
            ...
            $this->cleanup($node, $version, $home, $token);
    ...
    /** @mago-expect lint:sensitive-parameter The random lease token is an identifier, not a credential. */
    private function acquireTargetLock(Node $node, string $home, string $version, string $lockPath, string $token): void
    {
        $result = $this->execute($node, new RemoteCommand(
            arguments: ['/bin/bash', '-seu', '--', $home, $version, $lockPath, $token],
            input: <<<'BASH'
                home=$1; version=$2; lock_path=$3; token=$4
                test -d "$home"; test ! -L "$home"
                test "$(cd "$home" && pwd -P)" = "$home"
                test "$lock_path" = "$home/.orbit/run/php/php-fpm-$version.lock"
                for setup_directory in "$home/.orbit" "$home/.orbit/run" "$home/.orbit/run/php"; do
                ...
                for managed_directory in "$home/.orbit/php" "$home/.orbit/php/$version" "$home/.orbit/php/$version/versions" "$home/.orbit/logs" "$home/Library" "$home/Library/LaunchAgents"; do
        ...
        if (! $result->succeeded() || trim($result->stdout) !== 'ACQUIRED') {
            throw $this->failure('php-fpm-lock', 'app-dev.php_fpm_lock_failed', $result);
        }
    }
    ...
    private function releaseTargetLock(Node $node, string $lockPath, string $token): void
    {
        ...
        if (! $result->succeeded()) {
            throw $this->failure('php-fpm-lock-release', 'app-dev.php_fpm_lock_release_failed', $result);
        }
    }

    /**
     * @mago-expect lint:excessive-parameter-list Atomic publication needs each exact target and payload.
     * @mago-expect lint:sensitive-parameter The rendered PHP-FPM configuration contains paths but no credentials.
     */
    private function publish(
        ...
        $phpRoot = dirname($liveConfiguration);
        $rollback = "{$this->paths->home($node, RoleName::AppDev)}/.orbit/run/php/.rollback-{$version}-{$token}";
        ...
        $script = <<<BASH
            ...
            expected_label='com.orbit.php-fpm.{$version}'
            umask 077
            test -d "\$lock_path"
            test ! -L "\$lock_path"
            test "\$(cd "\$lock_path" && pwd -P)" = "\$lock_path"
            ...
            live_parent="\$php_root"
            test "\$rollback_parent" = "\$(dirname "\$lock_path")"
            for managed_directory in "\$php_root" "\$php_root/versions" "\$rollback_parent" "\$plist_parent"; do
                test -d "\$managed_directory"
                test ! -L "\$managed_directory"
                test "\$(cd "\$managed_directory" && pwd -P)" = "\$managed_directory"
            done
            candidate_directory="\$php_root/versions/\$token.candidate"
            published_directory="\$php_root/versions/\$token"
            candidate_configuration="\$candidate_directory/php-fpm.conf"
            candidate_plist="\$candidate_directory/$(basename "{$plist}")"
            published_configuration="\$published_directory/php-fpm.conf"
            ...
            validate_managed_directory "\$php_root/versions"
            validate_private_directory "\$candidate_directory"
            validate_private_directory "\$rollback"

            if [ -e "\$live_configuration" ] && cmp -s -- "\$candidate_configuration" "\$live_configuration" \
                && [ -e "\$plist" ] && cmp -s -- "\$candidate_plist" "\$plist"; then
            ...
            configuration_link="\$php_root/.php-fpm.\$token.link"
            ln -s -- "\$published_directory/php-fpm.conf" "\$configuration_link"
            switched=1
    ...
    private function restorePublishedState(
        ...
        try {
            $this->rollback($node, $version, $home, $plist, $token);
        } catch (Throwable $caught) {
        ...
        if ($rollbackFailure !== null) {
            throw new RuntimeConvergenceException(
                step: 'php-fpm-rollback',
                errorCode: 'app-dev.php_fpm_rollback_failed',
                message: 'The macOS PHP-FPM rollback failed.',
            );
        }
    }

    /** @mago-expect lint:sensitive-parameter The random publication token is an identifier, not a credential. */
    private function rollback(Node $node, string $version, string $home, string $plist, string $token): void
    {
        $live = $this->layout->phpCurrent($home, $version);
        $rollback = "{$home}/.orbit/run/php/.rollback-{$version}-{$token}";
        $lockPath = $this->layout->phpLock($home, $version);
        $result = $this->execute($node, new RemoteCommand(
            ...
            input: <<<'BASH'
                ...
                token=$5
                expected_user=$(/usr/bin/id -un)
                expected_label=$(/usr/bin/basename "$plist" .plist)
                case "$expected_label" in com.orbit.php-fpm.[0-9].[0-9]) ;; *) exit 76 ;; esac
                php_root=$(/usr/bin/dirname "$live_configuration")
                ...
                validate_current_publication() {
                    ...
                    if [ "$name" = 'configuration' ]; then
                        test -L "$path"
                        test "$(/usr/bin/stat -f '%Su' "$path")" = "$expected_user"
                        test "$(/usr/bin/readlink "$path")" = "$published_directory/php-fpm.conf"
                        return
                    fi
                    published_plist="$published_directory/$(/usr/bin/basename "$plist")"
        ...
        if (! $result->succeeded()) {
            throw $this->failure('php-fpm-rollback', 'app-dev.php_fpm_rollback_failed', $result);
        }
    }

    /** @mago-expect lint:sensitive-parameter The random publication token is an identifier, not a credential. */
    private function cleanup(Node $node, string $version, string $home, string $token): void
    {
        $rollback = "{$home}/.orbit/run/php/.rollback-{$version}-{$token}";
        $lockPath = $this->layout->phpLock($home, $version);
        $result = $this->execute($node, new RemoteCommand(
            ...
            input: <<<'BASH'
                ...
                token=$3
                expected_user=$(/usr/bin/id -un)
                lock_name=$(/usr/bin/basename "$lock_path")
                version=${lock_name#php-fpm-}; version=${version%.lock}
                case "$version" in [0-9].[0-9]) ;; *) exit 76 ;; esac
                expected_label="com.orbit.php-fpm.$version"
                home=${lock_path%/.orbit/run/php/*}
                test "$lock_path" = "$home/.orbit/run/php/php-fpm-$version.lock"
        ...
        if (! $result->succeeded()) {
            throw $this->failure('php-fpm-cleanup', 'app-dev.php_fpm_cleanup_failed', $result);
        }
    }
     ...
     * @mago-expect lint:sensitive-parameter The random lease token is an identifier, not a credential.
     */
    private function executeWithTargetLease(
        Node $node,
        array $arguments,
        string $lockPath,
        string $token,
        ...
        return $this->execute($node, new RemoteCommand(
            arguments: ['/bin/bash', '-seu', '--', $lockPath, $token, ...$arguments],
            input: <<<'BASH'
    ...
    private function brewPrefix(Node $node): string
    {
        return match ($node->architecture) {
            ...
            default => throw new RuntimeConvergenceException(
                step: 'php-version',
                errorCode: 'app-dev.php_architecture_unsupported',
                message: 'The Darwin PHP architecture is not supported.',
            ),
        };
    }
    ...
    private function plist(Node $node, string $version, string $formulaRoot, string $prefix, string $home): string
    {
        $label = "com.orbit.php-fpm.{$version}";
        $configuration = $this->layout->phpCurrent($home, $version);
        ...
        return <<<XML
            ...
            <string>{$formulaRoot}/sbin/php-fpm</string><string>--nodaemonize</string>
            <string>--fpm-config</string><string>{$configuration}</string>
            </array>
            <key>EnvironmentVariables</key><dict>
            <key>HOME</key><string>{$home}</string><key>USER</key><string>{$node->ssh_user}</string>
